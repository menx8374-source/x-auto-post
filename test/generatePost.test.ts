...
test("buildGenerationPromptはデフォルトアカウント(ジャンル省略時)で既存のジャンル説明文を維持する", () => {
  const c = candidate({ title: "Some AI news", url: "https://example.com/x" });
  const prompt = buildGenerationPrompt(c);
  assert.match(prompt.system, /あなたはAIニュースを紹介するXアカウントの編集者です。/);
});

test("エンゲージメント強化: システムプロンプトはリプライを誘発する工夫と、機械的な定型文の乱用を避ける指示を含む", () => {
  const c = candidate({ title: "Some AI news", url: "https://example.com/x" });
  const prompt = buildGenerationPrompt(c);
  assert.match(prompt.system, /リプライ/);
  assert.match(prompt.system, /機械的/);
});
...
test("複数アカウント対応: 未登録アカウント指定時のエラーメッセージはサフィックス付き環境変数名を案内する", async () => {
  const c = candidate({ title: "Some AI news", url: "https://example.com/x" });
  const testAccount: Parameters<typeof generatePostText>[2] = {
    id: "test-genre",
    label: "テストジャンル",
    genre: "テスト用の仮ジャンル",
    language: "ja",
    ...
    historyFilePath: "/tmp/does-not-matter.json",
  };
  const result = await generatePostText(c, null, testAccount);
  assert.equal(result.success, false);
  if (!result.success) {
    assert.match(result.error, /ANTHROPIC_API_KEY__TESTGENRE/);
  }
});
