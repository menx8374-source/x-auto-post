...
test("F12: assertValidConfigは空文字列のPOST_LANGUAGE/POST_TONEを検知する", () => {
  assert.throws(() => assertValidConfig({ POST_LANGUAGE: "   " } as NodeJS.ProcessEnv), ConfigError);
  assert.throws(() => assertValidConfig({ POST_TONE: "" } as NodeJS.ProcessEnv), ConfigError);
});
