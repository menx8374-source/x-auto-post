...
function buildMockDeps(overrides: Partial<AffiliatePipelineDependencies> = {}): {
  ...
  const deps: Partial<AffiliatePipelineDependencies> = {
    ...
    buildThread: (text, productId) => [
      { index: 1, text, charLength: text.length, kind: "body" },
      {
        index: 2,
        text: `商品ページ: https://menx8374-source.github.io/x-auto-post/go/${productId}.html`,
        charLength: `商品ページ: https://menx8374-source.github.io/x-auto-post/go/${productId}.html`.length,
        kind: "link",
      },
    ],
...
test("force相当: scheduledAt未指定でもslot指定があれば同日重複投稿防止(冪等性)は機能する(時間帯チェックのみスキップ)", async () => {
  const p = product({ id: "p1" });
  let generateCalled = false;
  const history: AffiliatePostHistoryEntry[] = [
    {
  ...
  const result = await runAffiliatePostingPipeline({
    ...
    publish: dryRunAffiliatePublish,
    slot: "affiliate",
    // scheduledAt未指定(--forceのCLI実装がこの形で呼ぶ) = 時間帯(不発リカバリ許容範囲)チェックは行わない
    now: new Date("2026-07-16T05:00:00.000Z"), // 通常の枠時刻(19:00 JST)とは無関係な時刻
    deps,
  });
...
test("force相当: scheduledAt未指定・slot指定ありで、同日投稿履歴が無ければ時間帯チェック無しで通常通り選定・生成される", async () => {
  const { deps } = buildMockDeps();
...
test("imageUrl: 商品にimageUrlが設定されている場合、OGP自動取得は行わずimageUrlを直接ダウンロードする", async () => {
  const p = product({ id: "p1", imageUrl: "https://example.com/p1-manual-image.jpg" });
  const sampleOgpImage: OgpImage = {
    url: "https://example.com/p1-manual-image.jpg",
    buffer: Buffer.from("fake-image-bytes"),
    contentType: "image/jpeg",
  };
  let downloadImageCalledWith: string | undefined;
  let fetchOgpImageCalled = false;
  const { deps } = buildMockDeps({
    loadProducts: async () => [p],
    select: () => ({ selected: p, reason: "テスト用選定", consideredCount: 1, enabledCount: 1 }),
    fetchOgpImage: async (url) => {
      fetchOgpImageCalled = true;
      return null;
    },
    downloadImage: async (url) => {
      downloadImageCalledWith = url;
      return sampleOgpImage;
    },
  });

  let receivedOgpImage: OgpImage | null | undefined;
  const publish: AffiliatePublishFn = async (tweets, publishedProduct, ogpImage) => {
    receivedOgpImage = ogpImage;
    return { posted: false, detail: "テスト" };
  };
  ...
  assert.equal(downloadImageCalledWith, "https://example.com/p1-manual-image.jpg");
  assert.equal(fetchOgpImageCalled, false);
  assert.equal(result.ogpImageUrl, sampleOgpImage.url);
  assert.equal(receivedOgpImage?.url, sampleOgpImage.url);
});

test("imageUrl: 商品にimageUrlが未設定の場合、従来通りofficialUrlに対してOGP自動取得を試みる(回帰なし)", async () => {
  const p = product({ id: "p1" }); // imageUrl未設定
  let fetchOgpImageCalledWith: string | undefined;
  let downloadImageCalled = false;
  const { deps } = buildMockDeps({
    loadProducts: async () => [p],
    select: () => ({ selected: p, reason: "テスト用選定", consideredCount: 1, enabledCount: 1 }),
    fetchOgpImage: async (url) => {
      fetchOgpImageCalledWith = url;
      return null;
    },
    downloadImage: async () => {
  ...
  assert.equal(fetchOgpImageCalledWith, "https://example.com/p1");
  assert.equal(downloadImageCalled, false);
  assert.equal(result.success, true);
});
