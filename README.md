...
- **商品情報**: `data/affiliate-products.json`(git管理対象)にユーザーが手動で商品を追加する。型定義は`src/affiliateProducts.ts`。各エントリ: `id`/`name`/`officialUrl`/`affiliateUrl`(ユーザーが手動入力するアフィリエイトリンク)/`facts`(事実ベースの特長の箇条書き、公式情報からユーザーが手動記入。AIは書かない)/`category`(任意)/`enabled`。初期状態は空配列で、商品が0件でもエラーにならず「投稿対象なし」として安全にスキップする。
  - **新しい商品を追加したら`npm run generate:affiliate-redirects`を実行する**: `src/generateAffiliateRedirects.ts`が`data/affiliate-products.json`の全商品ぶん、`docs/go/<productId>.html`という静的リダイレクトページ(`<meta http-equiv="refresh">`+JavaScriptフォールバック+手動リンクの3段構えで`affiliateUrl`へ遷移)を生成する。これはこのリポジトリで有効になっているGitHub Pages(`docs/`がソース、公開URL: `https://menx8374-source.github.io/x-auto-post/`)上に自前で設置するリダイレクトページで、ツイートには`https://menx8374-source.github.io/x-auto-post/go/<productId>.html`という固定URLを埋め込む(過去にTinyURL経由の短縮URLを使っていたが、非推奨API経由のリンクは開くと必ずTinyURL自身のPreview画面を経由してしまう不具合があったため廃止した)。**商品追加のたびにこのコマンドを実行し、`docs/go/`配下の生成物もコミットする必要がある**(自動生成されない)。
...
`src/adminTweetTool.ts`。不完全なスレッド投稿等の復旧作業のための最小限のCLIで、通常運用では使わない。既存の`X_API_KEY`等(認証情報系)をそのまま使う(新規の環境変数は無い)。

```bash
# 指定した1件のツイートを削除する(安全のため1回の実行につき1件のみ。一括削除は不可)
npm run admin:delete-tweet -- --id=<tweetId>

# 単発のテスト投稿を1件行う(スレッド化・返信化しない独立したツイート。文字数上限超過時はエラーで投稿しない)
npm run admin:test-post -- --text="<投稿するテキスト>"
```
