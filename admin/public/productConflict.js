...
// そのため、候補ヒント(src/generateCandidateHints.tsのproductCandidate)から自動生成した
// idが偶然、既存の有効な商品(facts/affiliateUrl設定済み)のidと一致すると、空のドラフト値で
// 警告なく上書き・データ消失してしまう。admin/public/app.jsはこの関数を使い、新規追加の
// 送信前にクライアント側で衝突を検知してブロックする。
