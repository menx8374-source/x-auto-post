...
test("validateApplicationTrackingInputは新規作成時、productNameは任意項目として扱う(未指定/nullは許容、指定する場合は空文字列を拒否)", () => {
  ...
  assert.equal(validateApplicationTrackingInput({ ...base, productName: null }).valid, true);
  assert.equal(validateApplicationTrackingInput({ ...base, productName: "" }).valid, false);
  assert.equal(validateApplicationTrackingInput({ ...base, productName: "   " }).valid, false);
  assert.equal(validateApplicationTrackingInput({ ...base, productName: "SuperAI Tool" }).valid, true);
});

test("validateApplicationTrackingInputは新規作成時、a8ProgramUrlが未指定/空文字列の場合拒否する", () => {
  const base = { productName: "SuperAI Tool" };
