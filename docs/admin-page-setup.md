...
商品候補(候補ヒントの「商品候補」バッジ)の横、および商品追加/編集フォームの公式サイトURL欄付近に「A8.netで探す」ボタンがある。押すと(a) A8.netのプログラム検索結果ページ(商品名をキーワードとして付与したURL)を新しいタブで開き、(b) 念のため商品名をクリップボードにもコピーし、(c) 案内メッセージを表示する。A8.netにログイン済みであればそのまま検索結果が表示され、未ログインの場合はA8.net自身の再認証画面が表示されるので、ログイン後にもう一度ボタンを押すか、コピー済みの商品名を検索欄に貼り付けて検索・提携申請すること(自動ログイン・自動申請は行わない)。
...
## 10. A8.net存在ヒント・提携申請の進捗管理

候補ヒントの「商品候補」に、A8.netに実在する可能性が高いかどうかの**ヒント**(断定ではない)を表示する機能。

**判定方法(いずれもA8.net自体へは一切アクセスしない、ヒューリスティック)**:
- **A8.net確認済み(主要ブランド)**(緑バッジ): 商品名がA8.net公式の公開ページ(ログイン不要)に実際に掲載されている主要ブランド広告主一覧(`src/a8NetHint.ts`の`KNOWN_A8_ADVERTISERS`)と一致した場合。
- **A8.netリンクを公式サイトで検知**(黄バッジ): 商品の公式サイト自体(A8.net自体ではない)のHTMLにA8.netドメインへのリンクが見つかった場合。
- **A8.net: 不明**(控えめな表示・バッジなし): 上記いずれにも一致しなかった場合。**「A8.netに存在しない」ことを意味しない**(単に自動判定できなかっただけ)。

上記2種類のヒントがある候補には「A8.netで確認・申請する」ボタンが表示される。押すと(a) A8.netの検索結果ページを新しいタブで開き、(b) 提携申請の進捗として「申請中」のエントリを記録する(A8.netへの自動ログイン・自動検索・自動提携申請は一切行わない)。

管理ページ下部の「提携申請の進捗」セクションで記録した申請の一覧を確認できる。**提携申請が実際に受理されたかどうかはユーザー本人がA8.netにログインしないと分からないため、自動検知はしない**。「申請中」のエントリを確認できたら「提携済みにする」ボタンで手動でステータスを切り替え、「提携済み」になったエントリの「商品を追加」ボタンから、既存の商品追加フォームを公式サイトURL・商品名を事前入力した状態で開ける(実際のアフィリエイトリンクはA8.netで作成後にユーザー自身が貼り付ける)。
