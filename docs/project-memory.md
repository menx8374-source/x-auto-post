...
- **アフィリエイトリンクを貼るだけで商品を自動入力・追加(2026-07-19)**: ユーザー要望「商品IDとか商品名とか必須にしないでください。必須なのはA8.netの『リンク先URLをコピー』で手に入るリンクだけです」に対応。新規`POST /api/resolveAffiliateLink`が、貼り付けられたA8.netアフィリエイトトラッキングリンクのリダイレクトを手動で1ホップずつ追跡(各ホップをfetchする前にSSRF安全性を検証、`suggestFacts.ts`より一段階厳格な事前検証方式)し、最終到達ページから商品名・画像(OGPメタデータ)・事実情報を自動抽出して商品追加フォームを事前入力する。貼り付けたアフィリエイトリンク自体(A8.netの成果計測パラメータを含む)は一切書き換えない。`/security-review`(指摘なし)・`/code-review`(low、1ラウンド目でCONFIRMEDな正しさのバグ2件(OGPメタタグ抽出の引用符処理・フォームのrequired属性による無言ブロック)を検出→修正→2ラウンド目でPASS)を実施。
- **A8.net存在ヒント+提携申請ステータス追跡(2026-07-19)**: ユーザー要望「候補ヒントにA8.netで実在する可能性が高いプログラムのみおすすめ表示、ワンボタンで申請ページへ、受理前後でステータスが変わるように」に対応。A8.netへの自動ログイン・自動検索・自動提携申請は実装しない方針を維持し、`src/a8NetHint.ts`(新規)で(a) A8.net公式公開ページの主要ブランド広告主リストとの名前一致、(b) 商品の公式サイト自体(A8.net自体ではない)へのA8.netリンクの有無スキャン、の2種のヒューリスティックな「ヒント」を`src/generateCandidateHints.ts`に追加(`src/ogpImage.ts`の既存SSRF対策済みfetchSafelyを再利用、ロジック変更なしexport追加のみ)。「不明」は「存在しない」を意味しないことを明示。新規`admin/functions/api/applicationTracking.ts`で提携申請ステータス(申請中/提携済み)をユーザーが手動管理できる「提携申請の進捗」セクションを管理ページに新設(受理判定はA8.netログインが必要なため自動検知しない)。`/security-review`(指摘なし)・`/code-review`(low、1ラウンド目でCONFIRMEDバグ1件(`officialUrlGuess`を持たないknown_brandヒントの申請記録が常に失敗)を検出→修正→2ラウンド目でPASS)を実施。
- **候補ヒント機能を完全削除しA8.net連携をシンプル化(2026-07-19)**: ユーザー方針転換により、AIニュースから商品候補を自動検出する「候補ヒント」機能一式(`src/generateCandidateHints.ts`・`src/a8NetHint.ts`・`admin/functions/api/candidates.ts`・`.github/workflows/update-candidate-hints.yml`・`data/affiliate-candidate-hints.json`・管理ページの「候補ヒント」セクション)を完全削除。代わりに(1)「カテゴリからA8.netを探す」(技術系/転職系/学習系/ビジネスツール系/電子製品系の固定キーワードでA8.net検索結果を新タブで開くだけ)、(2)A8.netプログラム詳細ページURL(ログイン後管理画面のページ、fetchは一切行わない)を貼り付けて`programId`をURLパースのみで抽出し提携申請の進捗を記録する機能、の2点に置き換えた。`ApplicationTrackingEntry`から`a8NetHint`/`officialUrl`を廃止し`a8ProgramId`/`a8ProgramUrl`に変更。`admin/functions/api/applicationTracking.ts`・`admin/functions/_lib/validate.ts`・新規`admin/functions/_lib/a8ProgramUrl.ts`を更新。`src/ogpImage.ts`の`fetchSafely`/`readTextBodyWithLimit`/`defaultFetch`/`defaultLookup`は外部利用箇所が無くなったため非exportに戻した。本番投稿パイプラインには一切手を加えていない。
- **アフィリエイト投稿ローテーション上限に例外を追加(2026-07-19)**: ユーザー要望「アフィリエイト投稿の対象が他にない場合は、同じアフィリエイトでも投稿上限は3回ではなく無制限にして」に対応。`src/selectAffiliateProduct.ts`で、上限判定後に選べる商品が1件も残らない場合(=他に投稿対象が無い場合)は上限を適用せず有効な商品全体から選び直すようにした(商品登録数が少ない運用初期に永久スキップされる問題を回避)。他に候補がある場合の既存ローテーション挙動は変更なし。あわせて「有効/無効の切り替えボタン」も要望されたが、既存の商品一覧「投稿対象」チェックボックスで既に満たされていることを確認しユーザーに回答、追加実装は不要と判断。レビューで指摘なし。
- **残タスク**:
  1. 商品の追加登録(ユーザー対応、実在のアフィリエイトプログラム登録が前提のため代行不可)。「アフィリエイトリンクを貼るだけで追加」機能・A8.netプログラム詳細ページURLからの提携申請記録により、候補発見からA8.net申請・登録までの操作は大幅に簡略化されたが、A8.net上での提携申請・審査・リンク取得自体は引き続きユーザー本人のみ対応可能。
  2. facts自動提案・アフィリエイトリンク自動解決機能(いずれもAnthropic API使用)を使うには、Cloudflare Pagesの環境変数に`ANTHROPIC_API_KEY`を追加登録する必要がある(未登録の場合、AI抽出部分のみ機能しないが、OGPメタデータからの商品名・画像自動入力自体は引き続き動作する)。
