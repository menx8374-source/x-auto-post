...
- **要検討: アフィリエイト収益化の依頼(2026-07-18、未実装)**: ユーザーから「収益はアフィリエイトをメイン」「実際の利用感想ではなく長所を述べ購買を促す文章」との追加依頼あり。旧プロジェクト(x-auto-post)は「体験談・アフィリエイト告知は虚偽体験談・誇大広告を避けるためAI自動生成から意図的に除外」という設計判断をしていた経緯があり、今回の依頼(実体験なしに購買誘導する文章)は景品表示法(ステルスマーケティング規制含む)上のリスクが高いと判断し、実装前にユーザーへ懸念点を共有し方針を確認中。
- **残タスク**: アフィリエイト機能の方針確定待ち。それ以外は運用フェーズ(継続監視はGitHub Actionsのジョブサマリー・通知設定、または`gh run list`等で確認)。
